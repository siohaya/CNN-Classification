        ...
        for i in range(k_fold):
            with tf.Session(graph=self.graph) as sess:
                ...
                if i==0:
                    feed_dict = {self.input_x: test_data, self.input_y: test_labels, self.dropout_keep_prob: 1.0}
                    #accuracy_out = sess.run(self.accuracy, feed_dict=feed_dict)
                    accuracy_out,correct_predictions,predictions,softmax= sess.run([self.accuracy,self.correct_predictions,self.predictions,self.softmax],feed_dict=feed_dict)
                    #print(correct_predictions,predictions,scores,softmax)
                    np.savetxt("test_labels0.csv",test_labels,fmt="%.1f,%.1f")
                    np.savetxt("correct_predictions0.csv",correct_predictions,fmt="%.1f")
                    np.savetxt("confidence0.csv", softmax,fmt="%.5f,%.5f")
                    np.savetxt("predicitions0.csv",predictions,fmt="%.1f")
                    print('Test accuracy: %.3f' % accuracy_out)
                final_accurary = final_accurary+accuracy_out
        print(final_accurary)
